...
class VisionTransformerUpcycle(nn.Module):
    """ Vision Transformer
    ...
    def forward_features(self, x, sigma=0):
        ...
        x = self.norm(x)
        if self.dist_token is None:
            return self.pre_logits(x[:, 0]), l_aux
        else:
            return (x[:, 0], x[:, 1]), l_aux

    def forward(self, x, sigma=0):
        x, l_aux = self.forward_features(x, sigma=sigma)
        if self.head_dist is not None:
            x, x_dist = self.head(x[0]), self.head_dist(x[1])  # x must be a tuple
            if self.training and not torch.jit.is_scripting():
                # during inference, return the average of both classifier predictions
                return x, x_dist, l_aux
            else:
                return (x + x_dist) / 2, l_aux
        else:
            x = self.head(x)
        return x, l_aux
...
@torch.no_grad()
def _load_weights(model: VisionTransformerUpcycle, checkpoint_path: str, prefix: str = ''):
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = build_model_with_cfg(
        VisionTransformerUpcycle, variant, pretrained,
