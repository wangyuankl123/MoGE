...
from .utils.layers import PatchEmbed, DropPath, AttentionPoolLatent, PatchDropout, \
    trunc_normal_, lecun_normal_, resample_patch_embed, resample_abs_pos_embed, use_fused_attn, \
    get_act_layer, get_norm_layer, LayerType
from .utils.manipulate import named_apply, checkpoint_seq, adapt_input_conv
...
try:
    ...
    tutel_moe = None
    raise ImportError("Tutel has not been installed. To use Swin-MoE, please install Tutel; otherwise, just ignore this.")
...
class MoEMlp(nn.Module):
    def __init__(self, in_features, hidden_features, num_local_experts, top_value, capacity_factor=1.25,
                 cosine_router=False, normalize_gate=False, use_bpr=True, is_gshard_loss=False,
                 gate_noise=1.0, cosine_router_dim=256, cosine_router_init_t=0.5, moe_drop=0.0, init_std=0.02,
                 mlp_fc2_bias=True,
                 is_group_sparsity_loss=False, gs_loss_weight=None, gs_map_size=None, gs_window_size=None,
                 gs_stride=None):
        super().__init__()
        ...
        self._moe_layer = tutel_moe.moe_layer(
            ...
            is_group_sparsity_loss=is_group_sparsity_loss, gs_loss_weight=gs_loss_weight, gs_map_size=gs_map_size,
            gs_window_size=gs_window_size, gs_stride=gs_stride)
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, mlp_ratio: float = 4., qkv_bias: bool = False, qk_norm: bool = False,
                 proj_drop: float = 0., attn_drop: float = 0., init_values: Optional[float] = None,
                 drop_path: float = 0., act_layer: nn.Module = nn.GELU, norm_layer: nn.Module = nn.LayerNorm,
                 is_moe=False, num_local_experts=1, top_value=1, capacity_factor=1.25, moe_drop=0.0,
        ...
        if self.is_moe:
            self.mlp = MoEMlp(in_features=dim,
                              ...
                              is_group_sparsity_loss=is_group_sparsity_loss, gs_loss_weight=gs_loss_weight,
                              gs_map_size=gs_map_size, gs_window_size=gs_window_size, gs_stride=gs_stride)
        else:
    ...
    def forward(self, x: torch.Tensor, sigma=0) -> torch.Tensor:
        ...
        if self.is_moe:
            x = self.norm2(x)
            x, l_aux = self.mlp(x, sigma)
            x = self.ls2(x)
            x = shortcut + self.drop_path2(x)
            return x, l_aux
        else:
            x = shortcut + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
            return x


class VisionTransformerMoGE(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
            ...
            num_local_experts=1, top_value=1, capacity_factor=1.25, moe_drop=0.0, aux_loss_weight=0.01, mode='Last2',

            is_group_sparsity_loss=False, gs_loss_weight=None, gs_window_size=None, gs_stride=None,
        ...
        """
        super().__init__()
        assert mode in ['Dense', 'Last2'], f"The current mode {mode} of VMoE has not been implemented yet"

        self._ddp_params_and_buffers_to_ignore = list()

        assert is_group_sparsity_loss, "The parameter is_group_sparsity of ViTTransformerMoGE should be True "
        ...
        gs_map_size = transform_to_2d_map(self.global_experts)
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                is_moe=True if (i == depth - 1 or i == depth - 3) and mode == "Last2" else False,
                num_local_experts=num_local_experts,
                top_value=top_value,
                capacity_factor=capacity_factor,
                moe_drop=moe_drop,
    ...
    def forward_features(self, x: torch.Tensor, sigma=0):
        l_aux = 0.0
    ...
    def forward(self, x: torch.Tensor, sigma=0):
        x, l_aux = self.forward_features(x, sigma)
...
@torch.no_grad()
def _load_weights(model: VisionTransformerMoGE, checkpoint_path: str, prefix: str = '') -> None:
