...
def main(config):
    # shutil.copy(f'train_moe.sh', os.path.join(config.OUTPUT, 'train_moe.sh'))
    shutil.copy(f'config.py', os.path.join(config.OUTPUT, 'config.py'))
    shutil.copy(f'main_swinmoge.py', os.path.join(config.OUTPUT, 'main_swinmoge.py'))
    ...
    shutil.copy(f'./models/swin_transformer_moge.py', os.path.join(config.OUTPUT, 'swin_transformer_moge.py'))
    ...
    sigma_scheduler = SigmaScheduler(config.MODEL.SWIN_MOGE.GS_SIGMA_FINAL, config.MODEL.SWIN_MOGE.GS_SIGMA_INIT,
                                     len(data_loader_train) * config.TRAIN.EPOCHS,
                                     gamma=config.MODEL.SWIN_MOGE.GS_SIGMA_POWER)
    ...
    save_checkpoint(config, 'final', model_without_ddp, max_accuracy, optimizer, lr_scheduler, loss_scaler,
                    logger, sigma_scheduler, zero_redundancy=True)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler,
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
            outputs, l_aux = model(samples, sigma_scheduler.get_sigma())
        ...
        # Temporarily increase the learning rate
        if epoch >= 140:
            optimizer.param_groups[0]['lr'] = optimizer.param_groups[0]['lr'] * 0.125
        elif epoch >= 115:
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            logger.info(
                ...
                f'Sigma {sigma_scheduler.get_sigma():.5f}'
            )
...
def validate(config, data_loader, model, sigma_scheduler):
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        # compute output
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
            output, l_aux = model(images, sigma_scheduler.get_sigma())
...
def throughput(data_loader, model, logger, sigma_scheduler):
    ...
    for idx, (images, _) in enumerate(data_loader):
        ...
        for i in range(50):
            model(images, sigma_scheduler.get_sigma())
        torch.cuda.synchronize()
