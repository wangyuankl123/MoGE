...
from . import losses
from . import loss_group_sparsity
...
class MOELayer(torch.nn.Module):
    ...
    def __init__(
        ...
        gs_padding_mode=None, gs_loss_mode='conv',
        ...
        **kwargs
        ...
        self.is_group_sparsity_loss = is_group_sparsity_loss
        self.gs_loss_weight = gs_loss_weight
        self.gs_map_size = gs_map_size
        self.gs_window_size = gs_window_size
    ...
    def forward(self, input: Tensor, gate_index=0, capacity_factor=None, top_k=None, a2a_ffn_overlap_degree=None, reserve_dims=1,
                inequivalent_tokens=False, adaptive_r=None, megablocks_size=0, sigma=0):
        if self.skip_moe:
            result_output = input
        ...
        def routing():
            ...
            elif self.is_group_sparsity_loss:
                _loss_fn = lambda gates, topk_ids: loss_group_sparsity.load_importance_sparsity_loss(
                    scores_wo_noise=F.softmax(logits, dim=1), topk_logits=logits_w_noise.gather(index=topk_ids, dim=1),
                    num_global_experts=self.num_global_experts, gate_noise=gctx.gate_noise,
                    map_size=self.gs_map_size, window_size=self.gs_window_size, stride=self.gs_stride,
                    gs_loss_weight=self.gs_loss_weight, gs_sigma=sigma
                )
            else:
