...
def _train_step(model, load_balance, X, Y, h_cache, eval_only, loss_div=1, sigma=0):
    """Single training step."""

    out, h_cache = model(X, h_cache, sigma=sigma)
    out = out.view(-1, out.size(-1))
    loss = torch.nn.functional.nll_loss(out, Y.view(-1))
    loss_value = loss.item() / loss_div
    # print('Updating Model: loss = ', loss)
...
def _train_batch(
    model, load_balance, optimizer, scheduler, X, Y, h_cache, eval_only, batch_split, sigma_scheduler=None
):
    """Train on a batch."""

    optimizer.zero_grad()
    curr_sigma = sigma_scheduler.get_sigma() if sigma_scheduler is not None else 0.01
...
def train_iteration(
    ...
    return loss_all, train_pos, h_cache, sigma_scheduler
