...
def _load_checkpoint(checkpoint_path, model, optimizer, scheduler, logger, distributed, sigma_scheduler):
    ...
    if "sigma_scheduler" in checkpoint_state:
        sigma_scheduler.load_state_dict(checkpoint_state["sigma_scheduler"])
    return iter_init


def load_checkpoint(checkpoint_path, model, optimizer, scheduler, logger, distributed, resume, sigma_scheduler):
...
def save_checkpoint(
    checkpoint_path, nb_batches_per_iter, model, optimizer, scheduler, logger, sigma_scheduler
):
    if checkpoint_path:
        checkpoint_state = {
        ...
        if sigma_scheduler is not None:
            checkpoint_state["sigma_scheduler"] = sigma_scheduler.state_dict()
...
class Logger:
    ...
    def log_iter(
        self, iter_no, nb_batches_per_iter, loss_train, loss_val, elapsed, model, sigma
    ):
