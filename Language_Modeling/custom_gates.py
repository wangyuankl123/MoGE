...
from loss_group_sparsity import load_importance_sparsity_loss
...
class CustomNaiveGate_Balance_SMoE(BaseGate):
    ...
    def set_load_balance(self, gate, gate_top_k_idx):
        print('Surprisingly, this run has started executing the set_load_balance function')
    ...
    def set_group_sparsity(self, gate, gslw, gs_sigma):

        score = F.softmax(gate, dim=-1)
        loss = load_importance_sparsity_loss(score, gs_loss_weight=gslw, gs_sigma=gs_sigma)

        self.loss = loss

    def forward(self, inp, gslw=0, gs_sigma=0, return_all_scores=False):
        gate = self.gate(inp)
        ...
        self.set_group_sparsity(gate, gslw, gs_sigma)
