...
class CustomizedMoEPositionwiseFF(FMoETransformerMLP):
    ...
    def forward(self, inp, sigma=0, gslw=0):

        if self.pre_lnorm:
            ##### layer normalization + positionwise feed-forward
            core_out = super().forward(self.layer_norm(inp), sigma=sigma, gslw=gslw)
            ...
            core_out = super().forward(inp, sigma=sigma, gslw=gslw)
            core_out = self.dropout(core_out)
...
class CustomizedMoEPositionwiseFFMoM(FMoETransformerMLP):
    ...
    def forward(self, inp, moment, sigma=0, gslw=0):
...
class TransformerSeqLayer(nn.Module):
    ...
    def forward(self, h, h_cache, moment, key_pe, sigma=0, gslw=0):
        # h = B x M x H
        # h_cache = B x L x H
        if self.use_attn:
        ...
        if self.use_smoe:
            if self.g == "m" or self.g == "a":
                smoe_out, moment = self.smoe(h, moment, sigma=sigma, gslw=gslw)
            elif self.g == "g":
                smoe_out = self.smoe(h, sigma=sigma, gslw=gslw)
...
class TransformerSeq(nn.Module):
    ...
    def forward(self, x, h_cache, sigma=0):
        # x size = B x M
        block_size = x.size(1)
        ...
        for l, layer in enumerate(self.layers):
            if layer.use_attn:
                ...
                h, moment = layer(h, h_cache[l], moment, self.key_pe, sigma=sigma, gslw=self.gslw)  # B x M x H
            else:
                h = layer(h, [], self.key_pe, sigma=sigma, gslw=self.gslw)
